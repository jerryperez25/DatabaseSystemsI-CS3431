...
--Natural Join
select firstName, lastName, tourName
from Reservation natural join Customer natural join Tour
where Tour.tourName = 'Duck Tour' or Tour.tourName = 'Golden Gate'
...
on R.customerID = C.CustomerID inner join Tour T
on R.tourID = T.tourID inner join Guide G
on R.guideID = G.guideID
where C.age > 21
and G.vehicleType = 'bus';

--Theta Joins pt 2
select distinct T.tourName, G.firstName, G.lastName
from Reservation R join Tour T
on R.tourID = T.tourID inner join Guide G
on R.guideID = G.guideID
order by T.tourName desc, G.lastName desc;
